...
def main():
    ...
    for acc in args.accs:
        ...
        data_train_path = Path('data') / args.tool / acc / "input_train.csv"
        data_train = pd.read_csv(data_train_path, sep=',')
        dfs_to_merge_train.append(data_train)
    ...
    if args.tool == "federated-svd":
        ...
        key = "msp_id"
        combo_df = pd.concat(
            [dfs_to_merge_train[0]] + [df.drop(columns=key) for df in dfs_to_merge_train[1:]],
            axis=1
        )
        combo_df.to_csv(combo_path / "input_train.csv", sep=',', index=False)

    elif args.tool == "random-forest":
        # combo_df = pd.concat(dfs_to_merge, axis=0)
        combo_df_train = pd.concat(dfs_to_merge_train, axis=0)
        combo_df_test = pd.concat(dfs_to_merge_test, axis=0)
        combo_df_train.to_csv(combo_path / "input_train.csv", sep=',', index=False)
